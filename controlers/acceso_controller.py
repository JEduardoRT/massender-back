...
def create_acceso(current_user: Annotated[User, Security(get_current_active_user)],
                  acceso: Acceso,
                  db: Session = Depends(get_db)):
    try:
        acceso.fecha_insercion = datetime.now(datetime.UTC)
...
def read_acceso(current_user: Annotated[User, Security(get_current_active_user)],
                acceso_id: int,
                db: Session = Depends(get_db)):
    try:
        acceso = db.query(AccesoRepo).filter(AccesoRepo.acceso_id == acceso_id).first()
...
def read_accesos(current_user: Annotated[User, Security(get_current_active_user, scopes=["admin"])],
                 db: Session = Depends(get_db)):
    try:
        accesos = db.query(AccesoRepo).all()
        return accesos
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def read_accesos_by_rol(current_user: Annotated[User, Security(get_current_active_user)],
                        rol_id: int,
                        skip: int = 0,
                        limit: int = 10,
                        db: Session = Depends(get_db)):
    try:
        accesos = db.query(AccesoRepo).join(AccesoRolRepo).filter(AccesoRolRepo.rol_id == rol_id).offset(skip).limit(limit).all()
        return accesos
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def update_acceso(current_user: Annotated[User, Security(get_current_active_user)],
                  acceso: Acceso,
                  db: Session = Depends(get_db)):
    try:
        acceso_existente = db.query(AccesoRepo).filter(Acceso.acceso_id == acceso.acceso_id).first()
        if not acceso_existente:
            raise HTTPException(status_code=404, detail="Acceso no encontrado")

        acceso.fecha_modificacion = datetime.now(datetime.UTC)

        for key, value in acceso.model_dump().items():
...
def delete_acceso(current_user: Annotated[User, Security(get_current_active_user)],
                  acceso_id: int,
                  db: Session = Depends(get_db)):
    try:
        acceso = db.query(AccesoRepo).filter(AccesoRepo.acceso_id == acceso_id).first()
